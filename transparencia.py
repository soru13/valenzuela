    ...
    for cur_row in range(salto_row, row_count):
        ...
        if cell.value:
            if(sheet2):
                ...
                sheet_transparencia_excel.cell(row=(maxima_row+cur_row-salto_row-1), column=45, value=file)
...
def iter_columnas(col_count , sheet, maxima_row, row_count, row, salto_inicio, book, sheet2, file):
    for cur_col in range(0, col_count):
        ...
        if (' Ejercicio' == cell.value):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 2, sheet2, True, file)
        if ((' Tipo de Procedimiento (catálogo)' == cell.value) or (' Tipo de Procedimiento' == cell.value)):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 3, sheet2, False, file)
        if (' Materia' == cell.value or ' Materia O Tipo de Contratación (catálogo)' == cell.value or ' Categoría:' == cell.value):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 4, sheet2, False, file)
        if (' Número de Expediente, Folio O Nomenclatura' == cell.value):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 5, sheet2, False, file)
        if (' Nombre(s) Del Adjudicado' == cell.value or ' Nombre(s) Del Contratista O Proveedor' ==  cell.value or ' Nombre(s)' == cell.value):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 6, sheet2, False, file)
        if (' Primer Apellido Del Adjudicado' == cell.value or ' Primer Apellido' == cell.value or ' Primer Apellido Del Contratista O Proveedor' == cell.value):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 7, sheet2, False, file)
        if (' Segundo Apellido Del Adjudicado' == cell.value or ' Segundo Apellido' == cell.value or ' Segundo Apellido Del Contratista O Proveedor' == cell.value):
        ...
        if (' Denominación O Razón Social' == cell.value or
            ...
            ' Nombre O Razón Social Del Adjudicado' in cell.value):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 9, sheet2, False, file)
        if (' Rfc de La Persona Física O Moral Contratista O Proveedor' == cell.value or ' Registro Federal de Contribuyentes (rfc) de La Persona Física O Moral Adjudicada' == cell.value):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 10, sheet2, False, file)
        if (' Área(s) Solicitante' == cell.value):
        ...
        if (' Número Que Identifique Al Contrato' == cell.value):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 13, sheet2, True, file)
        if (' Monto Del Contrato sin Impuestos (en Pesos Mex.)' == cell.value or ' Monto Del Contrato sin Impuestos (en Mxn)' == cell.value):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 14, sheet2, True, file)
        if (' Monto Total Del Contrato con Impuestos Incluidos' == cell.value or ' Monto Total Del Contrato con Impuestos Incluidos (mxn)' == cell.value):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 15, sheet2, True, file)
        if (' Tipo de Moneda' == cell.value):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 16, sheet2, False, file)
        if (' Tipo de Cambio de Referencia, en su Caso' == cell.value):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 17, sheet2, False, file)
        if (' Forma de Pago' == cell.value):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 18, sheet2, False, file)
        if (' Origen de Los Recursos Públicos (catálogo)' == cell.value or ' Origen de Los Recursos Públicos' == cell.value):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 19, sheet2, False, file)
        if (' Fuente de Financiamiento' == cell.value):
        ...
        if (' Descripción de Las Obras, Bienes O Servicios' == cell.value):
        ...
        if (' Fecha Del Contrato' == cell.value):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 24, sheet2, False, file)
        if (' Hipervínculo Al Comunicado de Suspensión, en su Caso' == cell.value):
        ...
        if (' Mecanismos de Vigilancia Y Supervisión de La Ejecución, en su Caso' == cell.value):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 40, sheet2, False, file)
        if (' Hipervínculo a Los Informes de Avance Financiero, en su Caso' == cell.value):
            insertar(row_count, cur_col, maxima_row, sheet, salto_inicio, 41, sheet2, False, file)
        if (' Monto Total de Garantías Y/o Contragarantías, en Caso de Que Se Otorgaran durante El Procedimiento' == cell.value):
